...
function VseriesRaw({allStock}) {
    const [data, setdata ] = useState([]);

    async function getData() {
        const url = "http://localhost:3000/stockBikes/v_series_raw";
...
const [value,setValue] = useState(allStock[8]?.image_url);
...
const options = [
    ...
    {label: "Purple", value:data[3]?.image_url}
]
...
function priceByMotor() {
    ...
    if (motor == data[0]?.motor) {
        ...
        price = data[3]?.price;
        range = data[3]?.autonomy;

        return (
        <>
                <h3>Range: {data[3]?.autonomy} Km</h3>   
                <h3 name="price"> Price: {data[3]?.price} €</h3>     
        </>   
        )}    
}
...
function handleSelect(event) {
    ...
    if (event.target.value == data[3]?.image_url) {
        setColor("Purple");
    }
...
<>
    <div className="specific-model-specs">
        ...
        <div className="motor-selector">
            ...
            <input type="radio" value={data[3]?.motor} name="motor" id="motor-500" onChange={(e) => setMotor(e.target.value)}/>
            <label htmlFor="motor-500">{data[3]?.motor} Watts</label>
        </div>
